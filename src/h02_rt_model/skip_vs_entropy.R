...
# input_fname <- '../../checkpoints/rt_and_entropy/rt_vs_entropy-provo4_skip2zero-gpt-small.tsv'
# input_fname <- '../../checkpoints/rt_and_entropy/rt_vs_entropy-dundee4_skip2zero-gpt-small.tsv'
# input_fname2 <- '../../checkpoints/rt_and_entropy/rt_vs_entropy-dundee5_skip2zero-gpt-small.tsv'
# input_fname3 <- '../../checkpoints/rt_and_entropy/rt_vs_entropy-dundee4-gpt-small.tsv'
# input_fname4 <- '../../checkpoints/rt_and_entropy/rt_vs_entropy-dundee5-gpt-small.tsv'
# is_linear <- TRUE
# merge_workers <- TRUE

set.seed(42)
df <- load_and_preprocess_data(input_fname, merge_workers)
# df2 <- load_and_preprocess_data(input_fname2, merge_workers)
# df3 <- load_and_preprocess_data(input_fname3, merge_workers)
# df4 <- load_and_preprocess_data(input_fname4, merge_workers)
...
if (is_linear & merge_workers) {
  ...
  for (baseline in baselines) {
    ...
    baseline_llhs[[baseline_name]] <- get_baseline_score(tgt_var, baseline_predictors, baseline_function, is_logistic=TRUE)
  }
...
print(baseline_llhs[['simple']])
print(mean(baseline_llhs[['simple']]))
# print(df$tgt_var)
print(mean((df$skipped * log(df$skipped)) +  ((1 - df$skipped) * log(1 - df$skipped))))
mean_skip = mean(df$skipped)
print(mean_skip* log(mean_skip) +  ((1 - mean_skip) * log(1 - mean_skip)))
...
for(i in 1:length(predictors)){
  ...
  if (is_linear && merge_workers) {
    ...
    list[full_llh,full_models] <- lme_cross_val(formula, df, tgt_var, random_effects=FALSE, is_logistic=TRUE)
  } else if (is_linear && !merge_workers) {
    sys.exit()
  } else if (!is_linear && merge_workers) {
    sys.exit()
